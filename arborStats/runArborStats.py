...
def compute_each_seg_id(seg_id):
    ...
    if "No meshes found." in result.stderr:
        ...
        if os.path.exists(skeleton_warped_path):
            with open(os.path.join(output_dir, "processed_seg_ids.txt"), "a") as f:
                f.write(f"{seg_id}\n")
...
if __name__ == "__main__":
    ...
    with open(os.path.join(output_dir, "processed_seg_ids.txt"), "w") as f:
        f.write(f"\n")

    # for seg_id in seg_ids[:5]:
    #     print(seg_id)
    #     result = subprocess.run(
    ...
    with mp.Pool(num_processes) as p:
        p.starmap(compute_each_seg_id, [(seg_id,) for seg_id in seg_ids])
